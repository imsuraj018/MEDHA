...
TRANSLATIONS.update({
    'ai_photo_screening': {'en': 'AI Photo Screening', 'mr': 'एआय फोटो तपासणी', 'hi': 'एआई फोटो स्क्रीनिंग'},
    'select_condition': {'en': 'Select condition', 'mr': 'स्थिती निवडा', 'hi': 'स्थिति चुनें'},
    'upload_photos': {'en': 'Upload newborn photos', 'mr': 'नवजात बाळाचे फोटो अपलोड करा', 'hi': 'नवजात शिशु की तस्वीरें अपलोड करें'},
    'screen_jaundice': {'en': 'Screen for Jaundice', 'mr': 'कावीळसाठी तपासणी करा', 'hi': 'पीलिया के लिए स्क्रीन करें'},
    'possible_jaundice': {'en': 'Possible jaundice finding', 'mr': 'कावीळची शक्यता आढळली', 'hi': 'पीलिया की संभावित पहचान'},
    'not_flagged': {'en': 'Not flagged by the model', 'mr': 'मॉडेलने चिन्हांकित केले नाही', 'hi': 'मॉडल द्वारा फ्लैग नहीं किया गया'},
    'jaundice_probability': {'en': 'Jaundice probability', 'mr': 'कावीळची संभाव्यता', 'hi': 'पीलिया की संभावना'},
    'multiple_photo_help': {'en': 'You can select multiple photos. Each image is screened separately and the combined summary is shown below.', 'mr': 'तुम्ही अनेक फोटो निवडू शकता. प्रत्येक फोटोची स्वतंत्र तपासणी केली जाईल आणि एकत्रित सारांश खाली दाखवला जाईल.', 'hi': 'आप कई तस्वीरें चुन सकते हैं। हर तस्वीर की अलग स्क्रीनिंग होगी और संयुक्त सारांश नीचे दिखाया जाएगा।'},
    'clinical_warning': {'en': 'This is an AI screening signal only, not a confirmed medical diagnosis. Clinical assessment should take precedence.', 'mr': 'हा फक्त एआय तपासणी संकेत आहे; निश्चित वैद्यकीय निदान नाही. वैद्यकीय तपासणीला प्राधान्य द्यावे.', 'hi': 'यह केवल एआई स्क्रीनिंग संकेत है, निश्चित चिकित्सीय निदान नहीं। चिकित्सीय मूल्यांकन को प्राथमिकता दी जानी चाहिए।'},
    'screening_summary': {'en': 'Screening Summary', 'mr': 'तपासणी सारांश', 'hi': 'स्क्रीनिंग सारांश'},
    'total_photos': {'en': 'Total Photos', 'mr': 'एकूण फोटो', 'hi': 'कुल तस्वीरें'},
    'flagged_photos': {'en': 'Flagged Photos', 'mr': 'चिन्हांकित फोटो', 'hi': 'फ्लैग की गई तस्वीरें'},
    'highest_probability': {'en': 'Highest Probability', 'mr': 'सर्वाधिक संभाव्यता', 'hi': 'सबसे अधिक संभावना'},
    'average_probability': {'en': 'Average Probability', 'mr': 'सरासरी संभाव्यता', 'hi': 'औसत संभावना'},
    'screening_history': {'en': 'Screening History', 'mr': 'तपासणी इतिहास', 'hi': 'स्क्रीनिंग इतिहास'},
    'photo_not_medical': {'en': 'Use clear newborn photographs. An unrelated image can still receive a model probability because this prototype is a binary classifier.', 'mr': 'स्पष्ट नवजात फोटो वापरा. हा प्रोटोटाइप बायनरी क्लासिफायर असल्यामुळे संबंधित नसलेल्या फोटोला देखील संभाव्यता मिळू शकते.', 'hi': 'स्पष्ट नवजात तस्वीरों का उपयोग करें। यह प्रोटोटाइप बाइनरी क्लासिफायर है, इसलिए असंबंधित तस्वीर को भी संभावना मिल सकती है।'},
})
...
if 'photo_screenings' not in st.session_state:
    st.session_state.photo_screenings = []
...
def call_openai_ai(question, history):
    ...
    instructions = AI_SYSTEM_PROMPT + f"\n\nRespond in {language}. Do not translate newborn IDs, dates, names, or database field values unless useful for readability.\n\nCURRENT APPLICATION DATA:\n{context}"
    if local_context:
        instructions += f"\n\nDETERMINISTIC QUERY RESULT FOR THIS REQUEST:\n{local_context}"

    # Keep the conversation context small and recent.
    recent = history[-8:]
    conversation = []
    for msg in recent:
        conversation.append({
    ...
    try:
        ...
        if not response.ok:
            ...
            return None, f'AI API error ({response.status_code}): {detail}'
        ...
        return None, f'Could not connect to the AI service: {exc}'
    except Exception as exc:
        return None, f'Unexpected AI response error: {exc}'
    ...
    if audio_file is not None:
        ...
        if st.session_state.get('voice_transcription_error'):
            ...
            st.info(
                ...
                'the problem is the OpenAI API key/model/access configuration.'
            )
...
with st.sidebar:
    ...
    st.divider()
    role_options = ['ASHA Worker', 'Taluka Officer', 'h Officer', 'State Admin', 'Super Admin']
    ...
    if st.button(f"📷 {t('ai_photo_screening')}", use_container_width=True):
        st.session_state.page = 'ai_photo_screening'
    if role != 'ASHA Worker':
...
def load_jaundice_model():
    model_path = os.path.join('models', 'jaundice_screening_model.keras')
    if not os.path.exists(model_path):
        return None
    return tf.keras.models.load_model(model_path)


def screen_jaundice_image(image):
    model = load_jaundice_model()
    if model is None:
        return None, ui_text('Jaundice model not found. Place jaundice_screening_model.keras inside the models folder.')
    try:
        rgb = image.convert('RGB')
        processed = rgb.resize((224, 224))
        ...
        result = 'Possible jaundice' if probability >= 0.50 else 'Not flagged'
        return probability, result
    except Exception as exc:
        return None, f'Image processing failed: {exc}'


def ai_photo_screening_page():
    st.title(f"📷 {t('ai_photo_screening')}")
    st.caption(ui_text('Select a condition and upload one or more newborn photographs for prototype AI screening.'))
    st.info(t('clinical_warning'))

    c1, c2 = st.columns([1, 2])
    with c1:
        condition = st.selectbox(t('select_condition'), ['Jaundice'], format_func=ui_text)
    with c2:
        st.markdown(f"**{t('upload_photos')}**")
        st.caption(t('multiple_photo_help'))

    uploaded_files = st.file_uploader(
        t('upload_photos'), type=['jpg', 'jpeg', 'png', 'webp'],
        accept_multiple_files=True, key='jaundice_multi_photo_uploader',
        help=t('photo_not_medical'),
    )

    if not uploaded_files:
        st.markdown(
            f"<div class='dashboard-card'><h3>📷 {ui_text('No photos selected')}</h3><p>{t('multiple_photo_help')}</p></div>",
            unsafe_allow_html=True,
        )
        return

    st.markdown(f"### {ui_text('Selected Photos')} ({len(uploaded_files)})")
    ...
    for i, uploaded in enumerate(uploaded_files):
        try:
            ...
            with preview_cols[i % len(preview_cols)]:
                ...
                st.caption(f"{preview.width} × {preview.height}px")
        except Exception:
            with preview_cols[i % len(preview_cols)]:
                st.error(f"{uploaded.name}: {ui_text('Invalid image')}")

    st.divider()
    if st.button(f"🔍 {t('screen_jaundice')}", type='primary', width='stretch'):
        if condition != 'Jaundice':
            st.warning(ui_text('Only Jaundice screening is currently available.'))
            return

        results = []
        progress = st.progress(0, text=ui_text('Analyzing photos...'))
        for index, uploaded in enumerate(uploaded_files, start=1):
            try:
                uploaded.seek(0)
                image = Image.open(uploaded).convert('RGB')
                probability, result = screen_jaundice_image(image)
                results.append({'file': uploaded.name, 'probability': probability, 'result': result})
            except Exception as exc:
                results.append({'file': uploaded.name, 'probability': None, 'result': f'Invalid image: {exc}'})
            progress.progress(index / len(uploaded_files), text=f"{ui_text('Analyzing photos...')} {index}/{len(uploaded_files)}")
        progress.empty()

        valid_results = [r for r in results if r['probability'] is not None]
        flagged = [r for r in valid_results if r['probability'] >= 0.50]
        if not valid_results:
            st.error(ui_text('No valid images could be analyzed.'))
            return

        screening_batch_id = f"SCR-{datetime.now().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:6].upper()}"
        for r in valid_results:
            st.session_state.photo_screenings.append({
                'screening_id': screening_batch_id, 'condition': condition,
                'file': r['file'], 'probability': r['probability'],
                'result': r['result'], 'screened_at': datetime.now(),
                'status': 'Pending clinical review',
            })

        st.markdown(f"### 📊 {t('screening_summary')}")
        m1, m2, m3, m4 = st.columns(4)
        with m1: st.metric(t('total_photos'), len(valid_results))
        with m2: st.metric(t('flagged_photos'), len(flagged))
        with m3: st.metric(t('highest_probability'), f"{max(r['probability'] for r in valid_results) * 100:.2f}%")
        with m4: st.metric(t('average_probability'), f"{sum(r['probability'] for r in valid_results) / len(valid_results) * 100:.2f}%")

        if flagged:
            st.warning(f"⚠️ {len(flagged)} {ui_text('photo(s) were flagged for possible jaundice.')} {t('clinical_warning')}")
        else:
            st.success(ui_text('None of the analyzed photos were flagged by the model.'))

        st.markdown(f"### 🖼️ {ui_text('Individual Results')}")
        for idx, r in enumerate(results, start=1):
            with st.container(border=True):
                rc1, rc2, rc3 = st.columns([2.4, 1.3, 1.8])
                with rc1: st.markdown(f"**{idx}. {r['file']}**")
                with rc2:
                    if r['probability'] is None: st.error(ui_text('Error'))
                    else: st.metric(t('jaundice_probability'), f"{r['probability'] * 100:.2f}%")
                with rc3:
                    if r['probability'] is None: st.caption(r['result'])
                    elif r['probability'] >= 0.50: st.warning(f"⚠️ {t('possible_jaundice')}")
                    else: st.success(f"🟢 {t('not_flagged')}")

        st.caption(f"Batch: {screening_batch_id} • {ui_text('Threshold')}: 50% • {ui_text('Prototype model; not clinically validated.')}")

    history = st.session_state.get('photo_screenings', [])
    if history:
        st.divider()
        st.markdown(f"### 🕘 {t('screening_history')}")
        history_df = pd.DataFrame(history)
        if not history_df.empty:
            display_df = history_df.copy()
            display_df['probability'] = display_df['probability'].map(lambda x: f'{x * 100:.2f}%' if pd.notna(x) else '-')
...
if page == 'dashboard':
    ...
    ai_assistant_page()
elif page == 'ai_photo_screening':
    ai_photo_screening_page()
elif page == 'research':
    research_page()
elif page == 'quality':
    data_quality_page()
